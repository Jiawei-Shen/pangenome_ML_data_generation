...
class Segment:
    ...
    def __init__(self, offset, seq, bq, cigar, rq, strand):
        ...
        self.cigar  = cigar    # bytes (unpadded; will be padded to node_length)
        self.rq     = rq       # int (MAPQ)
...
GLOBAL_MAJOR, GLOBAL_MINOR = 0, 3                        # bump minor for new layout (cigar = node_length)
...
def make_record_struct(node_length: int) -> struct.Struct:
    ...
    Layout: <h {L}s {L}s {L}s h c
      - i16 offset
      - seq[L] bytes
      - bq[L] bytes
      - cigar[L] bytes (ASCII, null-padded to node_length)
      - i16 rq (MAPQ)
      - char strand ('+' / '-')
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# CIGAR builder (unchanged semantics; now stored to node_length)
...
def process_alignment(raw_message, wanted_nodes, chrom_filter):
    ...
    for mapping in alignment.path.mapping:
        ...
        seg = Segment(
            ...
            cigar=cigar_string,     # will be padded/truncated to node_length
            rq=mapping_quality,
            strand=strand_char
        )
    ...
    with open(dat_path, "wb") as f:
        ...
        for node_id, info in block_infos.items():
            ...
            blank = rec_pack.pack(
                ...
                b'\x00' * node_len,
                b'\x00' * node_len,
                b'\x00' * node_len,  # cigar now node_len bytes
                0,
                b'+'
            )
        ...
        for node_id, segs in segment_buffer.items():
            ...
            for seg in segs:
                batch += rec_pack.pack(
                    ...
                    seg.cigar.ljust(node_len, b'\x00')[:node_len],  # cigar padded to node_len
                    int(seg.rq),
