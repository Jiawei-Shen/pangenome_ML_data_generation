...
# ─────────────────────────────────────────────────────────────────────────────
# Main filter (now supports MULTIPLE idx files)
# ─────────────────────────────────────────────────────────────────────────────
def filter_json_nodes_and_write(json_filepath, idx_filepaths, output_json_filepath,
                                vcf_file=None, txt_output_path=None):
    """
    Filter JSON 'nodes' by the union of node IDs found in one or more LATEST .idx files.

    :param json_filepath: path to input JSON
    :param idx_filepaths: iterable of .idx file paths (latest 30B format)
    :param output_json_filepath: path to output JSON
    :param vcf_file: optional VCF path for additional filtering
    :param txt_output_path: optional text output with final node IDs
    """
    ...
    print("\nStep 2: Loading node indices from ALL provided LATEST IDX files...")
    all_idx_node_ids = set()
    for idx_path in idx_filepaths:
        print(f"  Loading idx: {idx_path}")
        idx_data = load_index_latest(idx_path)
        ids_here = set(idx_data.keys())
        print(f"    Found {len(ids_here)} node IDs in {os.path.basename(idx_path)}")
        all_idx_node_ids.update(ids_here)

    total_idx_ids = len(all_idx_node_ids)
    print(f"\nCombined distinct node IDs across {len(idx_filepaths)} idx file(s): {total_idx_ids}")

    print("\nStep 3: Identifying common node IDs (JSON ∩ union(idx IDs))...")
    common_node_ids_int = target_node_ids_from_json.intersection(all_idx_node_ids)
    num_initial_common_nodes = len(common_node_ids_int)
    if num_initial_common_nodes == 0:
        print("No common node IDs found between JSON and all IDX files. Output 'nodes' list will be empty.")
    else:
        print(f"Found {num_initial_common_nodes} common node IDs between JSON and ALL idx files for potential processing.")
    ...
    if num_initial_common_nodes > 0:
        ...
        try:
            ...
            # fall back to keeping all common nodes
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter 'nodes' list in a JSON based on one or more LATEST .idx files (30B entries) "
                    "and optional VCF content via pysam."
    )
    parser.add_argument("json_path", help="Path to the input JSON file.")
    parser.add_argument(
        "idx_paths",
        nargs="+",
        help="Path(s) to LATEST .idx file(s) (30B entries). "
             "Provide one or more; all node IDs are unioned."
    )
    ...
    parser.add_argument(
        ...
        help="Optional: write final node IDs (one per line).",
        default=None
    )
    args = parser.parse_args()

    # Note: last positional is output_json_path; all but first & last are idx paths
    # Because of how we defined nargs, args.idx_paths already excludes json_path and output_json_path.
