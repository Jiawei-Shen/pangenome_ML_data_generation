...
# ─────────────────────────────────────────────────────────────────────────────
# Main processing function (REFACTORED TO USE PYSAM)
# ─────────────────────────────────────────────────────────────────────────────
def filter_json_nodes_and_write(json_filepath, idx_filepath, output_json_filepath, vcf_file=None, txt_output_path=None):
    ...
    if num_initial_common_nodes > 0:
        ...
        # This context manager opens the file ONCE and ensures it's closed.
        try:
                        ...
                        try:
                            ...
                            if start_0based_str is None or length_str is None:
                                ...
                                if length <= 0:
                                    ...
                                    # Perform the fast, in-process query using pysam
                                    # pysam.fetch() is 0-based and handles missing contigs gracefully
                            ...
                            # ValueError for int conversion, KeyError if contig not in VCF header
                            include_node_in_final_output = False
                            total_nodes_filtered_out_by_vcf_prereq += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter 'nodes' list in a JSON based on IDX file and VCF content using the 'pysam' library. Outputs modified JSON and optionally a TXT list of filtered node IDs.")
