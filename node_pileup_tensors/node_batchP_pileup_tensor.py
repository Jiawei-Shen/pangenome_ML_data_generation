...
import re
# ProcessPoolExecutor and as_completed are removed for serial execution
...
# Globals for worker process state are removed as it's now serial
    ...
    try:
        with open(idx_path, 'rb') as f:
            ...
            if file_size < expected_min_size:
                sys.stderr.write(
                    f"⚠️ Warning: Index file size ({file_size}) is smaller than expected ({expected_min_size}) for {num_nodes_in_idx} records.\n")
            ...
            for i in range(num_nodes_in_idx):
                ...
                if len(record_bytes) < 22:
                    sys.stderr.write(
                        f"❌ Error: Index file ended prematurely at record {i + 1}. Loaded {processed_entries} entries.\n")
                    break
...
# ─────────────────────────────────────────────────────────────────────────────
def process_node_serially(dat_file_handle, base_output_dir,  # Takes open DAT file handle
    ...
    try:
        ...
        dat_file_handle.seek(dat_file_offset + 10)
        for _ in range(n_records):
            data = dat_file_handle.read(RECORD_SIZE)
    ...
    # FileNotFoundError is handled by main, this catches read/seek errors
    except Exception as e:
    ...
    for (v_pos, v_type, v_ref, v_alt), _ in candidate_variants.items():
        ...
        view_reads_count = 0
        for seg in aligned_read_segments:
            if view_reads_count >= TENSOR_MAX_READ_ROWS: break
        ...
        for i in range(TENSOR_WINDOW_SIZE):
            abs_pos = win_start + i
            if 0 <= abs_pos < node_len: ref_bases_tensor[i] = BASE_TO_INDEX.get(node_sequence[abs_pos].upper(),
                                                                                BASE_TO_INDEX['N'])
        ...
        try:
            ...
            np_array = tensor.numpy()  # CONVERT TO NUMPY ARRAY
            fname = f"{key_str}.npy"  # USE .npy EXTENSION
            np.save(os.path.join(node_specific_output_dir, fname), np_array)  # SAVE AS .npy
...
def display_pileup_data(node_data_for_display_view, node_id_str_for_display, full_node_sequence,
    ...
    if not node_data_for_display_view:  # Check if dict is empty after initial check
...
def main():
    parser = argparse.ArgumentParser(
        description="Serially generate variant-centered .npy tensors and JSON summaries for specified node(s). Reads GFA and IDX once.",
    ...
    if needed_gfa_seqs and args.gfa:
        ...
        print(
            f"✔ GFA: Fetched {len(fetched)} new sequences in {time.time() - s_tm:.2f}s. Total in map: {len(seq_map)}.")
    ...
    # Determine verbosity: True if view is not 0 (None, -1, or >0 will be verbose)
    verbose_node_output = not (args.view == 0)

    # Open DAT file once for serial processing
    try:
        with open(args.dat, 'rb') as dat_file_handle:
            for i, node_id_int in enumerate(sorted_target_ids):
                ...
                if not dat_info or not seq:
                    if verbose_node_output:  # Still print skips if verbose, otherwise silent unless stderr
                        print(f"\n══════ SKIPPING NODE: {node_id_int} ({i + 1}/{len(sorted_target_ids)}) ══════")
                    if not dat_info: sys.stderr.write(f"❌ Node {node_id_int}: Not in IDX data. Skipping.\n")
                    ...
                    if nodes_in_current_batch_serial == 1000 or (i + 1) == len(sorted_target_ids):
                        if nodes_in_current_batch_serial > 0:
                            ...
                            print(
                                f"  Time for this batch: {batch_duration_serial:.2f}s ({nodes_per_sec_serial:.2f} nodes/sec)")
                            print(
                                f"  .npy files in this batch: {npy_in_current_batch_serial}")  # Will be 0 for purely skipped batch
                ...
                try:
                    offset, n_recs = dat_info  # Ensure offset and n_recs are defined for the call
                    _, view_data, npy_this_node = process_node_serially(
                        dat_file_handle, args.output, node_id_int, offset, n_recs, seq, args.min_af)
                    ...
                    elif verbose_node_output:
                        if view_data is not None and not view_data:
                            print(f"  ℹ️ No variants met AF for node {node_id_int}.")
                        elif view_data is None:
                            print(f"  ⚠️ Processing error for node {node_id_int} (returned no view data).")

                    if verbose_node_output and args.view is not None:  # Already know args.view != 0
                        if view_data:
                    ...
                    sys.stderr.write(f"❌ CRITICAL ERROR during processing of node {node_id_int}: {e}\n")
                    import traceback
                    traceback.print_exc(file=sys.stderr)  # Print full traceback for critical errors
                ...
                if nodes_in_current_batch_serial == 1000 or (i + 1) == len(sorted_target_ids):
                    if nodes_in_current_batch_serial > 0:
                        ...
                        nodes_per_sec_serial = nodes_in_current_batch_serial / batch_duration_serial if batch_duration_serial > 0 else 0
                        print(f"\n--- Batch Report (Serial Processing) ---")
                        print(f"  Processed/Attempted batch of {nodes_in_current_batch_serial} nodes.")
                        print(f"  Total iterated so far: {i + 1}/{len(sorted_target_ids)}")
                        print(
                            f"  Time for this batch: {batch_duration_serial:.2f}s ({nodes_per_sec_serial:.2f} nodes/sec)")
    ...
    except FileNotFoundError:  # For the main dat_file open
        sys.exit(f"❌ Error: Main DAT file not found at {args.dat}. Cannot proceed.")
    except Exception as e:  # Catch any other unexpected errors during the main loop setup
        sys.exit(f"❌ An unexpected error occurred in the main processing setup: {e}")
    ...
    # Nodes skipped is total targets minus those for which processing logic was invoked.
    skipped_before_processing_count = len(sorted_target_ids) - processed_count
    print(
        f"  Nodes skipped due to missing initial data (IDX/sequence) before core processing: {skipped_before_processing_count} node(s).")
