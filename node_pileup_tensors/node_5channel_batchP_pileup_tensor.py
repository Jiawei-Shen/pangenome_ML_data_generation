        ...
        if op in ('M', '=', 'X'):
            for i in range(L):
                ...
                if npos < node_seq_len and rpos < read_seq_len:
                    ...
                    # ignore SNVs involving N on either side
                    if nb == 'N' or rb == 'N':
                        continue
...
def get_allele_from_read_at_node_pos(read_offset_on_node, read_sequence,
    ...
    # keep track of the last aligned base-quality before any deletion
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate variant-centered tensors from latest .dat/.idx format (R,C per block) into sharded NPYs. Output tensors have 5 channels (no AF channel).",
