...
def process_single_node_for_pileup(task_args):
        ...
        for _ in range(n_records):
            ...
            if strand_char == '-':
                ...
                if current_offset_on_node < 0:
                    # Log restored by uncommenting
                    sys.stderr.write(
                        f"Warning [Node {node_id}, read with off {off_from_file}]: Invalid neg offset {current_offset_on_node} for '-' strand. Skipping.\n")
                    continue
    ...
    # Log restored
    print(candidate_variants, len(candidate_variants))
    ...
    for (v_pos, v_type, v_ref_from_cigar, v_alt_from_cigar), _ in candidate_variants.items():
        ...
        # Corrected: Use calculate_window_start to ensure tensor is always centered
        ...
        # Corrected: Loop over the calculated window range to handle negative start positions
...
def display_pileup_data(node_data_for_display_view, node_id_str_for_display, full_node_sequence,
    ...
    for i, variant_key in enumerate(sorted_variant_keys):
        ...
        # Corrected: Robust reference display that handles negative start positions
        ref_chars = []
        for j in range(window_start_pos, window_start_pos + TENSOR_WINDOW_SIZE):
            if 0 <= j < len(full_node_sequence):
                ...
                ref_chars.append(' ')  # Pad with spaces to match INDEX_TO_BASE_FOR_VIEW for padding
        ref_display = "".join(ref_chars)
