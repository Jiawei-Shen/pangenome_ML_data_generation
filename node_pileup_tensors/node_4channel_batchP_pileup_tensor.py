...
# This function is now corrected to precisely extract the quality of the allele bases.
def get_allele_from_read_at_node_pos(read_offset_on_node, read_sequence, read_quality_str, read_cigar_ops_decoded,
    ...
    for length, op in read_cigar_ops_decoded:
        if op in ('M', '=', 'X'):
            ...
            if current_node_pos <= target_node_pos < current_node_pos + length:
                ...
                if read_idx < len(read_sequence):
                    ...
                    # If we are querying an indel site, a match/mismatch means the read has the reference state
            ...
            # Insertion occurs after the base at current_node_pos - 1
            ...
            # Check if the target position falls within this deletion
            if current_node_pos <= target_node_pos < current_node_pos + length:
                if expected_var_type == 'I': return "OTHER_FOR_INDEL", None
                if expected_var_type == 'D':
                    # Check if this deletion in the read matches the expected deletion
        ...
        # Optimization: if we've passed the variant position, we can stop
    ...
    try:
        ...
        for _ in range(n_records):
            ...
            if strand_char == '-':
                ...
                qual_str = qual_str[::-1]
                original_decoded_cigar_ops.reverse()
                # This offset calculation for reverse strand is an approximation and might need refinement based on exact alignment logic.
                alignment_span_on_node = sum(l for l, op in original_decoded_cigar_ops if op in 'MDN=X')
                off_from_file = node_len - off_from_file - alignment_span_on_node

            aligned_read_segments.append({
                "offset_on_node": off_from_file,
                "read_sequence": seq,
                "processed_quality_str": qual_str,
                "cigar_ops": original_decoded_cigar_ops,
...
def display_pileup_data(node_data_for_display_view, node_id_str_for_display, full_node_sequence,
    ...
    if not node_data_for_display_view:
        print(f"Info: No pileup data to display for node {node_id_str_for_display} (no variants met all filters).")
        return
