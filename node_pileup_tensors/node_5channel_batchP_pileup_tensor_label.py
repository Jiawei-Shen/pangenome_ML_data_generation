...
def detect_shard_bases(data_dir: str):
    ...
    Note:
      This script does NOT inspect tensor contents or channel count.
      It works equally for upstream 5-channel tensors (no genome AF channel)
      or older 6-channel tensors.
    """
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Classify variant tensors from variant_summary.ndjson using VCF + ref JSON. "
            "Compatible with 5-channel tensor shards (genome population AF channel removed) "
            "and also with older 6-channel shards, because this script only writes labels."
        )
    )
    ...
    # Write *_labels.npy for each shard
