...
# When AF is missing: ch6 = 0 everywhere, and ONLY the variant column = 3
MISSING_AF_SENTINEL_AT_VARIANT = 3
...
# Per-base AF bins (0..7) as list/array, or None if AF truly missing
GLOBAL_NODE_AF_BINS: Dict[int, Optional[List[int]]] = {}
...
def load_full_idx_data(idx_path):
    ...
    try:
        with open(idx_path, 'rb') as f:
            ...
            processed_entries = 0
            # Each record: <I Q I I H> = 22 bytes
...
def normalize_af_to_bin8(af_field: Any, expected_len: int) -> Optional[List[int]]:
    """
    Returns a list[int] (0..7) of length==expected_len, or None if AF truly missing.
    - list -> bin floats to 0..7
    - string -> direct digits '0'..'7'
    - None/empty -> None
    """
    if af_field is None:
        return None
    ...
    if isinstance(af_field, list):
        out = [af_float_to_bin8(x) for x in af_field[:expected_len]]
...
def process_single_node_for_pileup(task_args):
    ...
    # AF bins (0..7) list/array, or None if missing
    genomead_af_bins = GLOBAL_NODE_AF_BINS.get(node_id, None)

    aligned_read_segments = []

    try:
        # Bulk read all records for this node and iter-unpack (fast)
        worker_dat_file.seek(dat_file_offset + 10)  # keep original behavior
    ...
    for (v_pos, v_type, v_ref_from_cigar, v_alt_from_cigar), _ in candidate_variants.items():
        ...
        # ch6 row: bin8 or "missing AF" behavior
        ch6_row = np.zeros(TENSOR_WINDOW_SIZE, dtype=np.int8)
        if genomead_af_bins is not None:
            # copy bins from per-base AF (0..7)
            L = len(genomead_af_bins)
            for i, node_pos_in_window in enumerate(range(window_start_pos, window_start_pos + TENSOR_WINDOW_SIZE)):
                if 0 <= node_pos_in_window < node_len and node_pos_in_window < L:
                    ch6_row[i] = int(genomead_af_bins[node_pos_in_window])
        else:
            # NO AF: only the candidate variant column = 3
            variant_window_index = v_pos - window_start_pos
            if 0 <= variant_window_index < TENSOR_WINDOW_SIZE:
                ch6_row[variant_window_index] = MISSING_AF_SENTINEL_AT_VARIANT
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate variant-centered tensors from alignment data (AF: bin8 only).",
    ...
    parser.add_argument("candidate_variants_json",
                        help="JSON file containing nodes/sequences (AF optional).")
    ...
    # Load candidate nodes (sequence + AF bins) into read-only globals
    node_sequences: Dict[int, str] = {}
    node_af_bins_data: Dict[int, Optional[List[int]]] = {}
    ...
    GLOBAL_NODE_AF_BINS = node_af_bins_data
