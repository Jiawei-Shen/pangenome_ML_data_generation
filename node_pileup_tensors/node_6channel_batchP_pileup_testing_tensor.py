...
def process_single_node_for_pileup(task_args):
    ...
    for (v_pos, v_type, v_ref_from_cigar, v_alt_from_cigar), _ in candidate_variants.items():
        ...
        for read_segment_idx, seg_data in enumerate(aligned_read_segments):
            if read_segment_idx >= TENSOR_MAX_READ_ROWS + 50:
                break
...
def main():
    ...
    # Load JSON once: we will *filter* during the full .idx pass
    node_sequences = {}
    node_af_data = {}
    ...
    print(f"JSON provides sequences for {len(node_sequences)} node IDs.")

    # Load the entire index and iterate ALL nodes in it
    ...
    for node_id, (offset, n_records) in full_idx_data.items():
        # Only process nodes that appear in the JSON (everything else: continue)
        sequence = node_sequences.get(node_id)
        if sequence is None:
            continue
        matched += 1
        af_list = node_af_data.get(node_id, [])
        tasks.append((node_id, offset, n_records, sequence, af_list,
                      args.min_af, args.min_variants, args.min_allele_bq, args.variant_type))

    if not tasks:
        sys.exit("No valid tasks to run (no node IDs from JSON found in the index).")

    print(f"Matched {matched} JSON nodes against {len(full_idx_data)} index entries.")
