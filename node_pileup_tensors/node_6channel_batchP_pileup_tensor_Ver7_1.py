...
def detect_variants_from_cigar(offset_on_node, cigar_ops_decoded, read_sequence, node_sequence):
    """
    Return variants in a consistent canonical tuple order:
        (v_pos, v_type, v_ref, v_alt)

    - X: v_ref = reference base (node), v_alt = read base
    - I: v_ref = anchor base (node), v_alt = inserted sequence (read)
    - D: v_ref = deleted reference sequence (node), v_alt = "*"
    """
    variants = []
        ...
        if op in ('M', '=', 'X'):
            for i in range(L):
                ...
                if npos < node_seq_len and rpos < read_seq_len:
                    nb = node_sequence[npos].upper()   # ref
                    rb = read_sequence[rpos].upper()   # alt
                    if nb != rb and op != '=':
                        variants.append((npos, 'X', nb, rb))
                else:
                    break
            node_pos += L
            read_pos += L
            ...
            variants.append((ref_anchor, 'I', ref_base, ins))
            read_pos += L
        elif op == 'D':
...
def get_allele_from_read_at_node_pos(read_offset_on_node, read_sequence,
    ...
    """
    For SNP/X:
      returns (base, base_quality)

    For I:
      - returns inserted sequence (and mean quality) when a matching insertion is anchored at target_node_pos
      - returns "REF_STATE_FOR_INDEL" (and anchor base quality) for reads that align through anchor without insertion

    For D:
      - expected_ref_allele_for_indel should be the deleted reference string (len = del_len)
      - returns "*" for reads that carry exactly this deletion starting at target_node_pos
      - returns "REF_STATE_FOR_INDEL" for reads that align through target_node_pos (no deletion)
    """
    ...
    del_len = None
    if expected_var_type == 'D' and expected_ref_allele_for_indel:
        del_len = len(expected_ref_allele_for_indel)
    ...
    for L, op in read_cigar_ops_decoded:
        if op in ('M', '=', 'X'):
            if npos <= target_node_pos < npos + L:
                ...
                if ridx < len(read_sequence):
                    ...
                    if expected_var_type == 'D':
                        # This read aligns through the deletion start position => supports REF state for the deletion event.
            ...
            # Deletion starts at current npos on the node
            if expected_var_type == 'D' and del_len is not None and npos == target_node_pos and L == del_len:
            ...
            # Generic handling when querying inside the deletion span
            if npos <= target_node_pos < npos + L:
                if expected_var_type == 'I':
                    return "OTHER_FOR_INDEL", None
                if expected_var_type == 'D':
                    # If the read has some deletion overlapping the position but not matching the expected event,
                    # treat as OTHER.
                    return "OTHER_FOR_INDEL", None
                return "*", None
...
def process_single_node_for_pileup(task_args):
    ...
    for (v_pos, v_type, v_ref_from_cigar, v_alt_from_cigar), _ in candidate_variants.items():
        ...
        if v_type == 'D':
            # Deletion: ref is the deleted reference string; alt is "*"
            exp_ref = v_ref_from_cigar
            exp_alt = "*"
